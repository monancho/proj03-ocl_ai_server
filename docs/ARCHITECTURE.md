...
| LangChain | 문제 생성 chain, structured output 처리 | 없음 |
...
| 3 | 12,000자 제한 적용 | 직접 입력은 오류, web/youtube는 warning 후 절단 |
| 4 | LangChain chain 실행 | 1회 재생성 시도 |
...
| 5 | low는 `allowed=true`, medium/high는 `allowed=false` 반환 |
...
| 생성 횟수 제한 | 서비스 사용자 인증 도입 후 | AI 서버가 아니라 백엔드 서버 책임 |
